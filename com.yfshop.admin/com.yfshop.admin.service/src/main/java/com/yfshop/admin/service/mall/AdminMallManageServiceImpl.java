...
import com.yfshop.code.manager.ItemSpecNameManager;
import com.yfshop.code.manager.ItemSpecValueManager;
...
public class AdminMallManageServiceImpl implements AdminMallManageService {
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public Void modifyCategorySort(Integer categoryId, Integer sort) {
        ItemCategory category = new ItemCategory();
        category.setId(categoryId);
        category.setUpdateTime(LocalDateTime.now());
        category.setSort(sort);
        int rows = categoryMapper.updateById(category);
        Asserts.assertTrue(rows > 0, 500, "编辑序号失败");
        return null;
    }
