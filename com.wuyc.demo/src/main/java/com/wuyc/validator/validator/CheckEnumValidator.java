...
public class CheckEnumValidator implements ConstraintValidator<CheckEnum, Object> {
     ...
     * 白名单，有效的枚举值约束
     */
    private final Set<Enum> whitelistEnumValues = new HashSet<>();
    ...
    private void initEnumValues(Class<? extends Enum> enumClass,
        ...
        try {
            ...
            for (Enum enumValue : enums) {
                Object enumCode = method.invoke(enumValue);
                if (enumCode instanceof Integer
                        || enumCode instanceof String) {
                    this.enumValues.put(enumCode.toString(), enumValue);
                }
            }
    ...
    private Enum parseEnum(Object code) {
        if (code instanceof Integer) {
            return this.enumValues.get(code.toString());
        }
